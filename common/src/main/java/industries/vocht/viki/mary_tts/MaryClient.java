...
public abstract class MaryClient {
	...
	public static MaryClient getMaryClient(MaryAddress serverAddress, boolean profile, boolean quiet) throws IOException {
		return new MaryHttpClient(serverAddress, profile, quiet);
	}

	/**
	 * perform tts using MaryTTS
	 * @param voice the voice to use, or prudence if null
	 * @param text the text to perform tts on
	 * @return the binary WAV file generated by the mary server
	 */
	public byte[] tts(String voice, String text) throws IOException {
		if ( text != null && text.trim().length() > 0 ) {
			if ( voice == null || voice.length() == 0 || voice.equals("alice")) {
				voice = "dfki-prudence";
			} else {
                voice = "dfki-spike";
            }
			try ( ByteArrayOutputStream outputStream = new ByteArrayOutputStream() ) {
				process(text, "TEXT", "AUDIO", "en_GB", "WAVE",
						voice, "", "", null, outputStream, 0L);
	 ...
	 *             if communication with the server fails
	 */
	protected MaryClient(MaryAddress serverAddress) throws IOException {
	 ...
	 *             if communication with the server fails
	 */
	protected MaryClient(MaryAddress serverAddress, boolean profile, boolean quiet) throws IOException {
		if (serverAddress != null) {
			data = new MaryFormData(serverAddress);
		} else {
			data = new MaryFormData(); // will try to get server address from system properties
	...
	protected final void initialise(boolean profile, boolean quiet) throws IOException {
		...
		String[] info;
		if (!beQuiet)
			System.err.println("Mary TTS client 1.0 (impl. 1.0)");
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		MaryClient mc = getMaryClient(null, false, false);
